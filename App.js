...
const enrichTheme = (theme) => ({
  ...
  greenLight: theme.actionLight || theme.scheduleCard,
  cream: theme.pageBg,
  ...
  blue: theme.action,
  gold: theme.warning,
  headerText: theme.headerText || theme.textPrimary,
  bodyText: theme.textPrimary,
  mutedText: theme.textSecondary,
  actionLight: theme.actionLight || theme.scheduleCard,
});
...
let MOCK_CUSTOMERS = [
  {
    id: "1",
    name: "Mike Reynolds",
    street: "142 Palmetto Ct",
    city: "Nocatee",
    state: "FL",
    zip: "32081",
    address: "142 Palmetto Ct, Nocatee, FL 32081",
    phone: "(904) 555-0182",
  ...
  },
  {
    id: "2",
    name: "Sarah Chen",
    street: "88 Harbour View Dr",
    city: "Fleming Island",
    state: "FL",
    zip: "32003",
    address: "88 Harbour View Dr, Fleming Island, FL 32003",
    phone: "(904) 555-0247",
  ...
  {
    id: "3",
    name: "Dave & Lisa Torres",
    street: "310 Creekside Blvd",
    city: "St. Johns",
    state: "FL",
    zip: "32259",
    address: "310 Creekside Blvd, St. Johns, FL 32259",
    phone: "(904) 555-0391",
  ...
  },
  {
    id: "4",
    name: "Brenda Walsh",
    street: "57 Oak Hollow Ln",
    city: "Orange Park",
    state: "FL",
    zip: "32065",
    address: "57 Oak Hollow Ln, Orange Park, FL 32065",
    phone: "(904) 555-0554",
...
let MOCK_JOBS = [
  ...
  },
  {
    id: "j3",
    customerId: "3",
    customerName: "Dave & Lisa Torres",
    service: "Driveway Seal",
    sqft: 600,
    status: "paid",
    date: "2026-04-08",
    amount: 900,
  ...
  return (
    {
      ...
      "Pool Deck": "Pool Deck Sealing",
      Patio: "Patio Sealing",
      Walkway: "Walkway Sealing",
    }[type] || "Paver Sealing"
  );
}
        ...
        >
          {t.fab ? (
            <View style={styles.fabBtn}>
              <Text
                style={{ color: C.headerText, fontSize: 28, lineHeight: 32 }}
              >
                +
              </Text>
            </View>
            ...
            <Text
              style={[styles.tabIcon, active === t.key && { opacity: 0.4 }]}
            >
              {t.icon}
            </Text>
          )}
          ...
          <View style={{ gap: 10 }}>
            {ACTIONS.map((a) => (
              <TouchableOpacity
                ...
                style={[
                  styles.quickRow,
                  a.highlight && styles.quickRowHighlight,
                ]}
                onPress={() => {
                  onClose();
...
const HomeScreen = ({ navigate, params }) => {
  ...
  const activeQuotes = MOCK_JOBS.filter(
    (j) => j.status === "pending" && new Date(j.date) >= thirtyDaysAgo,
  );
  const unpaidTotal = activeQuotes.reduce((s, j) => s + j.amount, 0);
  const unpaidFmt =
    unpaidTotal >= 1000
      ? `$${(unpaidTotal / 1000).toFixed(1)}k`
      : `$${unpaidTotal}`;
  const dueCt = MOCK_CUSTOMERS.filter((c) => c.status === "due").length;
  const todayJobs = MOCK_JOBS.filter((j) =>
    ["scheduled", "active", "pending"].includes(j.status),
  );
  ...
  const MENU_ITEMS = [
    {
      ...
      desc: "Name, logo, contact, rates",
    },
    {
      icon: "📱",
      label: "Social Setup",
      screen: "SocialSetup",
      desc: "GBP, Instagram, Facebook links",
    },
    {
      icon: "🎄",
      label: "Holiday Campaigns",
      screen: "HolidayCampaigns",
      desc: "Seasonal outreach templates",
    },
    {
      icon: "🏷️",
      label: "Specials & Sales",
      screen: "Specials",
      desc: "Promotions & discount offers",
    },
    {
      icon: "🤝",
      label: "Referral Program",
      screen: "ReferralTracking",
      desc: "Referral rewards & tracking",
    },
    {
      icon: "📊",
      label: "Revenue Report",
      screen: "RevenueDashboard",
      desc: "Monthly & YTD performance",
    },
    {
      icon: "📅",
      label: "Follow-Up Schedule",
      screen: "FollowUpScheduler",
      desc: "Automate client touchpoints",
    },
    {
      icon: "🏘",
      label: "Customers Due",
      screen: "CustomersDue",
      desc: "Rebook pipeline",
    },
    {
      icon: "🔔",
      label: "Notifications",
          ...
          {ATTENTION.map((item) => (
            <View
              ...
              style={[
                ...
                { borderLeftColor: item.actionColor },
              ]}
            >
              <View
              ...
              >
                <View
                  style={[
                    styles.attentionDot,
                    { backgroundColor: item.actionColor + "22" },
                  ]}
                >
                ...
                <TouchableOpacity
                  style={{
                    backgroundColor: item.actionColor + "18",
            ...
            return (
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() =>
                    Linking.openURL(
                      `https://maps.google.com/maps?q=${encodeURIComponent(client?.address || job.customerName)}`,
                    )
                  }
                  style={{
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {MENU_ITEMS.map((item, i) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    if (item.screen) navigate(item.screen);
                      ...
                      Alert.alert("Log Out", "Sign out of JobTap?", [
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    <Text
                      style={{
                        fontSize: 15,
                        fontWeight: "700",
                        color: item.danger ? C.red : C.grey,
                      }}
                    >
                      {item.label}
                    </Text>
                  ...
                  <Text style={{ color: C.textSecondary, fontSize: 18 }}>
                    ›
                  </Text>
                </TouchableOpacity>
              ))}
...
const CustomersScreen = ({ navigate, customers = MOCK_CUSTOMERS }) => {
  ...
  const FILTERS = [
    ...
    {
      label: `VIP ${customers.filter((c) => c.status === "vip").length}`,
      key: "VIP",
      color: "#8B5CF6",
    },
    {
      label: `No review ${customers.filter((c) => c.status === "lead").length}`,
      key: "No review",
      color: "#F97316",
    },
    {
      label: `Booked ${customers.filter((c) => c.status === "active").length}`,
      key: "Booked",
      color: "#3B82F6",
    },
  ];
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.pageBg }}>
      ...
      >
        ...
        >
          ...
          >
            <Text
              style={{ color: C.headerText, fontSize: 13, fontWeight: "700" }}
            >
              + Add
            </Text>
          </TouchableOpacity>
        ...
        >
          <TextInput
            style={{ fontSize: 14, color: C.headerText }}
            placeholder="Search clients..."
            placeholderTextColor={`${C.headerText}80`}
            value={search}
              ...
              >
                <Text
                  style={{
                    fontSize: 12,
                    fontWeight: "700",
                    color: active ? C.headerText : C.grey,
                  }}
                >
                  {f.label}
                </Text>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.pageBg }}>
      ...
      <View style={{ backgroundColor: C.primaryBg, paddingBottom: 20 }}>
        ...
        >
          ...
          >
            <Text style={{ color: `${C.headerText}CC`, fontSize: 18 }}>‹</Text>
            <Text
              style={{
                color: `${C.headerText}CC`,
                fontSize: 13,
                fontWeight: "600",
              }}
            >
        ...
        <View style={{ alignItems: "center" }}>
          ...
          >
            <Text
              style={{ fontSize: 26, fontWeight: "800", color: C.headerText }}
            >
              {initials}
            </Text>
          </View>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          >
            <Text
              style={{ color: C.textPrimary, fontSize: 22, fontWeight: "900" }}
            >
        ...
        <View style={{ padding: 14, gap: 10 }}>
          {[
            ...
            [
              {
                label: "New quote",
                onPress: () => navigate("QuoteBuilder", { customer }),
              },
              {
                label: "Review ask",
                onPress: () => navigate("ReviewRequest", { customer }),
              },
            ],
          ].map((row, ri) => (
            <View key={ri} style={{ flexDirection: "row", gap: 10 }}>
              {row.map((btn) => (
                <Pressable
                  ...
                  style={({ pressed }) => ({
                    flex: 1,
                    backgroundColor: pressed ? C.green : C.greenLight,
                    borderRadius: 12,
                    paddingVertical: 14,
                    alignItems: "center",
                  })}
                >
                  {({ pressed }) => (
                    <Text
                      style={{
                        color: pressed ? C.headerText : C.green,
                        fontSize: 15,
                        fontWeight: "700",
                      }}
                    >
              ...
              >
                ...
                <View
                  style={{
                    ...
                    borderRadius: 20,
                    borderWidth: 1,
                    borderColor: job.status === "paid" ? C.green : C.orange,
                  }}
                >
                  <Text
                    style={{
                      color: job.status === "paid" ? C.green : C.orange,
                      fontSize: 11,
                      fontWeight: "700",
                    }}
                  >
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.pageBg }}>
      ...
      >
              ...
              <View style={styles.qbRow}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <TextInput
                    ...
                    autoCapitalize={
                      f.key === "email" || f.key === "phone" ? "none" : "words"
                    }
                    value={form[f.key]}
                    onChangeText={(v) => {
                      if (f.key === "phone") upd("phone", formatPhone(v));
                      else upd(f.key, v);
                    }}
                  />
                </View>
        ...
        <Card style={{ padding: 0, overflow: "hidden" }}>
          <View style={styles.qbRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <TextInput
                style={styles.qbRowValue}
                placeholder="142 Palmetto Ct"
                placeholderTextColor={C.greyMid}
                value={form.street}
                onChangeText={(v) => upd("street", v)}
              />
            </View>
          </View>
          ...
          >
            <View style={styles.qbRowIcon}>
              <Text style={{ fontSize: 15 }}>
                {source ? SOURCES.find((s) => s.key === source)?.icon : "📣"}
              </Text>
            </View>
            <View style={{ flex: 1 }}>
            ...
            <Text
              style={{ fontSize: 18, color: C.textSecondary, marginRight: 4 }}
            >
              ›
            </Text>
          </TouchableOpacity>
          ...
          >
            ...
            {SOURCES.map((s, i) => (
              ...
              >
                ...
                <Text style={{ fontSize: 16, color: C.dark, flex: 1 }}>
                  {s.label}
                </Text>
                {source === s.key && (
        ...
        <TouchableOpacity
          style={[styles.greenBtn, saving && { opacity: 0.6 }]}
          onPress={handleSave}
          disabled={saving}
        >
          {saving ? (
            ...
            <Text style={styles.greenBtnText}>Add Client →</Text>
          )}
        </TouchableOpacity>
        <View style={{ height: 40 }} />
      </ScrollView>
...
const QuoteBuilderScreen = ({
        ...
        prev.map((s) =>
          ...
          (params.measuredSurface === "Driveway" && s.label === "Paver Sealing")
            ? {
                ...s,
                selected: true,
                sqft: String(params.measuredSqft),
                open: false,
              }
            : s,
        ),
  ...
  const updateSqft = (id, val) =>
    setServices((prev) =>
      prev.map((s) => (s.id === id ? { ...s, sqft: val } : s)),
    );
  ...
  const updateUnit = (id, val) =>
    setServices((prev) =>
      prev.map((s) =>
        s.id === id ? { ...s, unit: val, sqft: "", price: "" } : s,
      ),
    );
  const handleSqftDone = (id) =>
    setServices((prev) =>
      prev.map((s) => {
        if (s.id !== id) return s;
  ...
  const deliveryLabel = {
    ...
    both: "Send Quote — Text + Email",
    draft: "Save as Draft",
  };
  const deliverySub = {
    text: "Delivered by text instantly",
    email: "Delivered by email instantly",
    both: "Delivered instantly by text + email",
    draft: "Saved — send later",
  };
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      ...
      Alert.alert(
        "✅ Quote Sent!",
        `${fmtCurrency(total)} quote sent to ${selectedCustomer.name}`,
      );
      navigate("Home");
    }, 900);
  };
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      <ScrollView
        style={[styles.body, { backgroundColor: C.pageBg }]}
        keyboardShouldPersistTaps="handled"
        showsVerticalScrollIndicator={false}
      >
        {!selectedCustomer ? (
        ...
        <Card style={{ padding: 0, overflow: "hidden" }}>
          ...
          >
            ...
            >
              ...
              <Text
                style={[styles.qbRowLabel, { marginBottom: 0, fontSize: 12 }]}
              >
                Service type — tap to select
              </Text>
            </View>
            {services.map((s) => {
              const lineTotal =
              ...
              const isFilled =
                s.selected && s.price && (s.unit === "flat" || s.sqft);
                    ...
                    {s.custom ? (
                      ...
                      <Text
                        style={{
                          flex: 1,
                          fontSize: 14,
                          color: s.selected ? C.grey : C.greyMid,
                    ...
                    <View style={styles.accordionBody}>
                      <TouchableOpacity
                        onPress={() =>
                          updateUnit(
                            s.id,
                            s.unit === "flat" ? "/sq ft" : "flat",
                          )
                        }
                        style={styles.flatToggleRow}
                      >
                        <View
                          style={[
                      ...
                      <View style={[styles.accordionRow, { marginTop: 8 }]}>
                        ...
                        <View style={styles.accordionInput}>
                          ...
                          <TextInput
                            ...
                            placeholder={
                              s.unit === "flat" ? "0.00" : s.defaultPrice || "0"
                            }
                            placeholderTextColor={C.greyMid}
                          />
                        </View>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => handleSqftDone(s.id)}
                      >
                        <Text style={[styles.greenBtnText, { fontSize: 14 }]}>
                          Done ✓
                        </Text>
                      </TouchableOpacity>
                    </View>
          ...
          <View style={styles.qbRow}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <TextInput
                style={[styles.qbRowValue, { minHeight: 40 }]}
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            navigate("MeasureEntry", { customer: selectedCustomer })
          }
        >
          <Text style={{ fontSize: 28 }}>📏</Text>
          <View style={{ flex: 1 }}>
            <Text
              style={{ color: C.headerText, fontSize: 14, fontWeight: "800" }}
            >
              Tap to Measure This Job
            </Text>
            <Text
              style={{
                color: `${C.headerText}A6`,
            ...
            >
              Satellite trace or camera tap · sq ft auto-fills
            </Text>
          </View>
          <Text style={{ color: C.headerText, fontSize: 18 }}>→</Text>
        </TouchableOpacity>
        ...
        <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
          {[
            ...
            <TouchableOpacity
              ...
              style={[
                styles.deliveryBtn,
                deliveryMethod === m.key && styles.deliveryBtnActive,
              ]}
            >
              <Text
                style={[
      ...
      <Modal visible={showQuotePreview} animationType="slide">
        <SafeAreaView style={{ flex: 1, backgroundColor: C.pageBg }}>
          ...
          >
            <View style={styles.quoteDoc}>
              ...
              <View style={styles.quoteFromTo}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.quoteFromDetail}>
                ...
                <View style={{ flex: 1, alignItems: "flex-end" }}>
                  ...
                  <Text style={styles.quoteFromName}>
              ...
              <View style={{ marginTop: 16 }}>
                <View style={styles.quoteLineHeader}>
                  <Text style={[styles.quoteSmLabel, { flex: 1 }]}>
                    SERVICE
                  </Text>
                  <Text
                    style={[
            ...
            <TouchableOpacity
              style={[styles.greenBtn, sent && { opacity: 0.6 }]}
              onPress={handleSend}
              disabled={sent}
            >
              {sent ? (
  ...
  try {
    ...
    if (fromCamera) {
      ...
      if (perm.status !== "granted") {
        Alert.alert("Camera needed", "Allow camera access in Settings.");
        return;
      }
    } else {
      perm = await ImagePicker.requestMediaLibraryPermissionsAsync();
      if (perm.status !== "granted") {
        Alert.alert("Photos needed", "Allow photo library access in Settings.");
        return;
      }
    }
...
const ActiveJobScreen = ({ navigate, params }) => {
  ...
  const handleDoneWithPhotos = () => {
    if (beforePhotos.length === 0) {
      Alert.alert("No photos", "Take at least one before photo.");
      return;
    }
    setPhotosTaken(true);
    startTimers();
    ...
    <SafeAreaView style={styles.screenGreen}>
      <View style={styles.header}>
        <View style={styles.headerRow}>
          ...
          >
            <Text
              style={{ color: C.headerText, fontSize: 11, fontWeight: "700" }}
            >
              ● In Progress
            </Text>
          </View>
      ...
      >
        ...
        >
          ...
          >
            {jobClient?.street} · {job.service}
          </Text>
          <View style={{ flexDirection: "row", gap: 8, marginTop: 14 }}>
            {[
              { label: "Quote", val: fmtCurrency(job.amount) },
              { label: "Started", val: photosTaken ? fmtTime(elapsed) : "—" },
              { label: "Sq ft", val: job.sqft || "—" },
            ].map((k) => (
        ...
        <View style={{ padding: 14, gap: 12 }}>
            ...
            >
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    You've arrived — take before photos
                  </Text>
                  <Text
                    style={{
                      color: `${C.headerText}99`,
                  ...
                  >
                    GPS detected · Job site confirmed
                  </Text>
                </View>
              </View>
              ...
              <View style={{ flexDirection: "row", gap: 8 }}>
                <TouchableOpacity
                  onPress={() => pickPhoto(true, addBefore)}
                  style={{
                    flex: 1,
                    backgroundColor: C.cardBg,
                    paddingVertical: 10,
                ...
                <TouchableOpacity
                  onPress={() => pickPhoto(false, addBefore)}
                  style={{
                    flex: 1,
                    backgroundColor: "rgba(255,255,255,0.15)",
                    paddingVertical: 10,
            ...
            >
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    Before photos · {beforePhotos.length} saved
                  </Text>
                  <Text style={{ color: `${C.headerText}A6`, fontSize: 12 }}>
                    Job clock started · {fmtTime(elapsed)}
                  </Text>
                </View>
                <TouchableOpacity
                  onPress={() => setPhotosTaken(false)}
                  style={{
                    backgroundColor: "rgba(255,255,255,0.15)",
                    paddingHorizontal: 10,
                    paddingVertical: 6,
          ...
          <TouchableOpacity
            style={[styles.greenBtn, { paddingVertical: 18 }]}
            onPress={() => navigate("JobComplete", { job, beforePhotos })}
          >
            <Text style={[styles.greenBtnText, { fontSize: 17 }]}>
              Mark Job Done
            </Text>
            <Text
              style={{
            ...
            >
              Triggers after photo flow + Invoice
            </Text>
          </TouchableOpacity>
        </View>
        <View style={{ height: 40 }} />
...
const JobsScreen = ({ navigate }) => {
  ...
  const activeJobs = MOCK_JOBS.filter(
    (j) =>
      ["scheduled", "active", "overdue"].includes(j.status) &&
  ...
  const outstandingQuotes = MOCK_JOBS.filter(
    (j) => j.status === "pending" && new Date(j.date) >= thirtyDaysAgo,
        ...
        <View style={styles.headerRow}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.headerSub}>
              {MOCK_JOBS.length} total · {fmtCurrency(totalPaid)} collected
            </Text>
          </View>
          <TouchableOpacity
          ...
          >
            <Text style={{ color: C.headerText, fontSize: 22 }}>+</Text>
          </TouchableOpacity>
        </View>
        <View
          style={[
            styles.headerSearch,
            { backgroundColor: "rgba(232,245,227,0.25)", marginTop: 14 },
          ]}
        >
          <Text
            style={{
          ...
          <TextInput
            style={{ flex: 1, fontSize: 14, color: C.headerText }}
      ...
      >
                ...
                >
                  ...
                  >
                    <Text
                      style={[
                        styles.avatarText,
                        { color: statusColor(item.status), fontSize: 14 },
                      ]}
                    >
                      {initials}
                    </Text>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.clientAddr}>
                      {item.service} · {fmtDate(item.date)}
                    </Text>
                  </View>
        ...
        >
          ...
          {paidJobs.slice(0, 3).map((j) => (
            ...
            >
              <Text style={{ fontSize: 13, color: `${C.headerText}CC` }}>
                {j.customerName}
              </Text>
              <Text
            ...
            {anySelected && (
              <TouchableOpacity
                onPress={() =>
                  Alert.alert(
                    "Resend Quotes?",
                    `Resend ${Object.values(selected).filter(Boolean).length} quote(s)?`,
                    [
                      { text: "Cancel", style: "cancel" },
                      {
                        text: "Resend",
              ...
              >
                ...
                >
                  📤 Resend ({Object.values(selected).filter(Boolean).length})
                </Text>
              </TouchableOpacity>
            )}
          </View>
                ...
                >
                  ...
                  <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
                    <TouchableOpacity
                      style={[
                        styles.greenBtn,
                        { flex: 1, marginBottom: 0, paddingVertical: 8 },
                      ]}
                      onPress={() =>
                        Alert.alert(
                          "Quote resent!",
                          `Sent to ${item.customerName}`,
                        )
                      }
                    >
                      <Text style={[styles.greenBtnText, { fontSize: 13 }]}>
                        📤 Resend
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
...
const JobCompleteScreen = ({ navigate, params }) => {
  ...
  const customer =
    MOCK_CUSTOMERS.find((c) => c.id === job.customerId) || MOCK_CUSTOMERS[0];
  ...
  const SplitCard = () => (
    ...
    >
      <View style={{ flexDirection: "row", height: 160 }}>
        ...
        >
          {beforeUri && !beforeUri.startsWith("mock") ? (
            <Image
          ...
          >
            <Text
              style={{ color: C.headerText, fontSize: 10, fontWeight: "800" }}
            >
              BEFORE
            </Text>
          </View>
    ...
    >
      <View style={{ height: 160, flexDirection: "row", gap: 2 }}>
        ...
        >
          ...
          >
            <Text
              style={{ color: C.headerText, fontSize: 9, fontWeight: "800" }}
            >
              BEFORE
            </Text>
          </View>
            ...
            >
              {uri && !uri.startsWith("mock") ? (
                ...
                <View
                  style={{
                    ...StyleSheet.absoluteFillObject,
                    backgroundColor: i === 0 ? C.green : C.greenDark,
                    justifyContent: "center",
                    alignItems: "center",
                  }}
                >
  ...
  const StoryCard = () => (
    ...
    >
      ...
      <View style={{ height: 2, backgroundColor: C.cardBg }} />
      ...
      >
        <Text style={{ fontSize: 13, fontWeight: "800", color: C.headerText }}>
          {customer.name}
        </Text>
        <Text style={{ fontSize: 11, color: `${C.headerText}A6` }}>
          HydroSeal Pavers
        </Text>
      </View>
    </View>
  ...
  const CardPreview = () => {
    if (cardTemplate === "split") return <SplitCard />;
    if (cardTemplate === "collage") return <CollageCard />;
    return <StoryCard />;
  };
      ...
      >
        <View style={{ padding: 14, gap: 12 }}>
          ...
          >
            ...
            <View style={{ flexDirection: "row", gap: 8 }}>
              <TouchableOpacity
                onPress={() => pickPhoto(true, addAfter)}
                style={{
                  flex: 1,
                  backgroundColor: C.cardBg,
                  paddingVertical: 10,
              ...
              <TouchableOpacity
                onPress={() => pickPhoto(false, addAfter)}
                style={{
                  flex: 1,
                  backgroundColor: "rgba(255,255,255,0.15)",
                  paddingVertical: 10,
              ...
              >
                ...
                <Text
                  style={{
                    fontSize: 10,
                    fontWeight: "700",
                    color: cardTemplate === t.key ? C.green : C.greyMid,
                    textAlign: "center",
                  }}
                >
                  {t.label}
          ...
          <CardPreview />
          <TouchableOpacity
            style={[styles.greenBtn, { paddingVertical: 18, marginTop: 4 }]}
            onPress={() => navigate("Invoice", { job, customer })}
          >
            <Text style={[styles.greenBtnText, { fontSize: 17 }]}>
              Send Card + Create Invoice
            </Text>
            <Text
              style={{
                color: "rgba(255,255,255,0.7)",
            ...
            >
              Shares {cardTemplate} card · opens invoice next
            </Text>
          </TouchableOpacity>
        </View>
        <View style={{ height: 40 }} />
...
const InvoiceScreen = ({ navigate, params }) => {
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      setSending(false);
      navigate("YouGotPaid", { job, customer, total });
    }, 1200);
  };
  return (
    <SafeAreaView style={styles.screen}>
      <Header
        title="Invoice"
        subtitle={`#INV-${job.id.toUpperCase()}`}
        onBack={() => navigate("ActiveJob", { job })}
      />
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
        <Card>
          ...
          >
            ...
            <View style={{ alignItems: "flex-end" }}>
              ...
              <Text
                style={{ fontSize: 13, color: C.orange, fontWeight: "600" }}
              >
                Upon Receipt
              </Text>
            </View>
        ...
        <Card>
          <View style={{ flexDirection: "row", marginBottom: 8 }}>
            ...
            <Text style={[styles.cardLabel, { width: 60, textAlign: "right" }]}>
              QTY
            </Text>
            <Text style={[styles.cardLabel, { width: 55, textAlign: "right" }]}>
              RATE
            </Text>
            <Text style={[styles.cardLabel, { width: 65, textAlign: "right" }]}>
              TOTAL
            </Text>
          </View>
        ...
        <View style={{ flexDirection: "row", gap: 8, marginBottom: 10 }}>
          {[
            { key: "card", label: "💳 Card", sub: "Square" },
            { key: "cash", label: "💵 Cash", sub: "Mark paid" },
            { key: "venmo", label: "📱 Venmo", sub: "Send link" },
            { key: "check", label: "📝 Check", sub: "Record" },
          ].map((m) => (
            <TouchableOpacity
              key={m.key}
            ...
            >
              ...
              <Text
                style={{
                  fontSize: 11,
                  fontWeight: "700",
                  color: payMethod === m.key ? C.green : C.grey,
                }}
              >
                {m.label.split(" ")[1]}
              </Text>
        ...
        <TouchableOpacity
          style={[styles.greenBtn, sending && { opacity: 0.7 }]}
          onPress={handleSend}
          disabled={sending}
        >
          {sending ? (
...
const YouGotPaidScreen = ({ navigate, params }) => {
  ...
  const payTime = new Date().toLocaleTimeString("en-US", {
    hour: "numeric",
    minute: "2-digit",
  });
  return (
    <SafeAreaView style={[styles.screenGreen, { backgroundColor: C.action }]}>
      ...
      >
        ...
        <View style={{ alignItems: "center", paddingHorizontal: 24 }}>
          ...
          >
            {customer.name} · {job.service} · {fmtDate(job.date)}
          </Text>
          <View
            style={{
          ...
          >
            ...
            />
            <Text
              style={{ color: C.headerText, fontSize: 13, fontWeight: "600" }}
            >
              Follow-up schedule set · running in background
            </Text>
          </View>
        </View>
        <View
        ...
        >
          ...
          <TouchableOpacity
            ...
            onPress={() => navigate("ReviewRequest", { job, customer })}
          >
            ...
            >
              Schedule running silently in background
            </Text>
          </TouchableOpacity>
          <View style={{ height: 20 }} />
        </View>
...
const ReviewRequestScreen = ({ navigate, params }) => {
  ...
  const [message, setMessage] = useState(
    `Hey ${firstName} — we really enjoyed working on your driveway. Would you mind leaving us a quick Google review? It means a lot to a small business like ours. Tap the link below — takes 30 seconds. 🙏`,
  );
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      Alert.alert(
        "✅ Review request sent!",
        `${customer.name} will receive it via ${channelSummary}.`,
  ...
  return (
    <SafeAreaView style={styles.screenGreen}>
      <View style={styles.header}>
        <View style={styles.headerRow}>
          <TouchableOpacity
            onPress={() => navigate("YouGotPaid", { job, customer })}
            style={styles.backBtn}
          >
            <Text style={styles.backArrow}>‹</Text>
          </TouchableOpacity>
        ...
        >
          {job.service} · Paid {fmtDate(job.date)}
        </Text>
        <View style={{ flexDirection: "row", gap: 8, marginTop: 10 }}>
          <View
            style={{
          ...
          >
            <Text
              style={{ color: C.headerText, fontSize: 11, fontWeight: "600" }}
            >
              Sends {autoDate}
            </Text>
          </View>
              ...
              >
                ...
                <Text
                  style={{
                    fontSize: 12,
                    fontWeight: "700",
                    color: channels[ch.key] ? C.green : C.grey,
                  }}
                >
                  {ch.label}
                </Text>
          ...
          >
            {sent ? (
              ...
              <>
                ...
                >
                  Or auto-fires {autoDate} · Day 3 from paid date
                </Text>
              </>
            )}
          </TouchableOpacity>
...
const FollowUpSchedulerScreen = ({ navigate, params }) => {
  ...
  const scColor = (s) =>
    ({ done: C.green, next: C.orange, queued: C.greyMid })[s] ||
    THEMES.rugged.greyMid;
  const scLabel = (s) =>
    ({ done: "Done", next: "Next", queued: "Queued" })[s] || s;
  return (
    <SafeAreaView style={styles.screenGreen}>
      <View style={styles.header}>
        <View style={styles.headerRow}>
          <TouchableOpacity
            onPress={() => navigate("Home", { openMenu: true })}
            style={styles.backBtn}
          >
            <Text style={styles.backArrow}>‹</Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            onPress={() => setAutoSet((v) => !v)}
            style={{
              backgroundColor: autoSet ? "#22C55E" : "rgba(255,255,255,0.2)",
              paddingHorizontal: 10,
              paddingVertical: 5,
              borderRadius: 12,
            }}
          >
            <Text
              style={{ color: C.headerText, fontSize: 12, fontWeight: "700" }}
            >
              {autoSet ? "Auto-set" : "Manual"}
            </Text>
          </TouchableOpacity>
        </View>
        <Text
        ...
        >
          Schedule started {fmtDate(jobDate)}
        </Text>
        <View style={{ flexDirection: "row", gap: 8, marginTop: 12 }}>
          ...
          >
            <Text
              style={{ color: C.headerText, fontSize: 12, fontWeight: "600" }}
            >
          ...
          {nextStep && (
            ...
            >
              <Text
                style={{ color: "#FED7AA", fontSize: 12, fontWeight: "600" }}
              >
                Next {fmtDate(nextStep.date)}
              </Text>
            </View>
          )}
        </View>
      ...
      >
        <View style={{ padding: 14 }}>
              ...
              <View style={{ alignItems: "center", width: 20 }}>
                <View
                  style={{
                    ...
                    borderRadius: 6,
                    backgroundColor:
                      step.status === "queued" && !step.enabled
                        ? C.border
                        : scColor(step.status),
                    marginTop: 14,
                    borderWidth: step.status === "queued" ? 2 : 0,
                    borderColor: scColor(step.status),
                  }}
                />
                {i < steps.length - 1 && (
                  <View
              ...
              <View style={[styles.card, { flex: 1, marginBottom: 8 }]}>
                ...
                >
                  <View style={{ flex: 1 }}>
                    ...
                    <Text
                      style={{
                        fontSize: 14,
                        fontWeight: "700",
                        color: step.enabled ? C.grey : C.greyMid,
                        marginTop: 2,
                      }}
                    >
                      {step.label}
                  ...
                  >
                    <View
                      style={{
                        backgroundColor: scColor(step.status) + "22",
                        paddingHorizontal: 8,
                        paddingVertical: 3,
                        borderRadius: 8,
                      }}
          ...
          <TouchableOpacity
            style={[styles.greenBtn, { marginTop: 8 }]}
            onPress={() => Alert.alert("✅ Schedule saved!")}
          >
...
const CustomersDueScreen = ({ navigate }) => {
  ...
  const CC = ({ c, badgeColor, badgeText }) => (
    <View
      style={[
        styles.card,
        { borderLeftWidth: 4, borderLeftColor: badgeColor, marginBottom: 10 },
      ]}
    >
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
        <View style={[styles.avatar, { backgroundColor: badgeColor + "22" }]}>
      ...
      <View style={[styles.header, { paddingBottom: 18 }]}>
        <View style={styles.headerRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.headerSub}>
              {overdue.length + dueNow.length + due7d.length} need attention
            </Text>
          </View>
        </View>
        <View style={{ flexDirection: "row", gap: 8, marginTop: 12 }}>
          {[
            { label: "Overdue", count: overdue.length, color: C.red },
            { label: "Due Now", count: dueNow.length, color: C.orange },
            { label: "Due in 7d", count: due7d.length, color: C.blue },
          ].map((pill) => (
            <View
              key={pill.label}
              style={{
            ...
            >
              <Text
                style={{ color: C.headerText, fontSize: 22, fontWeight: "900" }}
              >
                {pill.count}
              </Text>
              <Text
      ...
      >
        <View style={{ padding: 14 }}>
          ...
          {due7d.length > 0 && (
            <>
              {
                <Text style={[styles.sectionTitle, { color: C.blue }]}>
                  DUE IN 7 DAYS · {due7d.length}
                </Text>
              }
              {due7d.map((c) => (
                <CC key={c.id} c={c} badgeColor={C.blue} badgeText="Due soon" />
              ))}
            </>
          )}
        </View>
...
const RevenueDashboardScreen = ({ navigate }) => {
  ...
  const unpaidJobs = MOCK_JOBS.filter(
    (j) => j.status === "overdue" || j.status === "pending",
  );
    ...
    <SafeAreaView style={styles.screenGreen}>
      <View style={[styles.header, { paddingBottom: 14 }]}>
        <View style={styles.headerRow}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[styles.headerTitle, { fontSize: 24 }]}>Revenue</Text>
          </View>
          <View
            style={{
              flexDirection: "row",
            ...
            {["Day", "Week", "Month"].map((p) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  period === p && { backgroundColor: C.cardBg },
                ]}
              >
                <Text
                  style={{
                    fontSize: 12,
                    fontWeight: "700",
                    color: period === p ? C.green : "rgba(255,255,255,0.7)",
                  }}
                >
                  {p}
                </Text>
        ...
        <View style={{ alignItems: "center", paddingVertical: 16 }}>
          ...
          >
            +18% vs last month · $9,610
          </Text>
        </View>
        <View style={{ flexDirection: "row", gap: 6 }}>
          {[
            { label: "Today", val: "$480" },
            { label: "This week", val: "$4,200" },
            { label: "Jobs paid", val: "8" },
            { label: "Unpaid", val: fmtCurrency(unpaidTotal), red: true },
          ].map((k) => (
            <View
              key={k.label}
              style={{
        ...
        <View style={{ padding: 14 }}>
                  ...
                  >
                    <View
                      style={{
                        width: "100%",
                        height: h,
                        backgroundColor: i === 2 ? C.green : C.greenLight,
                        borderRadius: 6,
                      }}
                    />
                    <Text
          ...
          >
            ...
            <Text style={{ fontSize: 12, color: C.red, fontWeight: "700" }}>
              {fmtCurrency(unpaidTotal)} outstanding
            </Text>
          </View>
          {unpaidJobs.length === 0 ? (
            <Card>
            ...
            unpaidJobs.map((j) => {
              const days = Math.floor(
                (new Date() - new Date(j.date)) / 86400000,
              );
              return (
                <View key={j.id} style={[styles.card, { marginBottom: 10 }]}>
                  ...
                  >
                    <View
                      style={[styles.avatar, { backgroundColor: C.red + "22" }]}
                    >
                      <Text style={[styles.avatarText, { color: C.red }]}>
                        {j.customerName
                          .split(" ")
                          .map((n) => n[0])
                          .join("")
                  ...
                  <TouchableOpacity
                    style={[
                      styles.greenBtn,
                      { marginTop: 10, marginBottom: 0, paddingVertical: 8 },
                    ]}
                    onPress={() => Alert.alert("Invoice resent!")}
                  >
                    <Text style={[styles.greenBtnText, { fontSize: 13 }]}>
                      📤 Resend Invoice
                    </Text>
...
const NewLeadScreen = ({ navigate }) => {
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      Alert.alert(
        "✅ Lead Saved!",
        `${form.name || "New lead"} added.${autoReply ? "\n\nAuto-reply sent via text." : ""}`,
        [
          { text: "Build Quote", onPress: () => navigate("QuoteBuilder") },
          { text: "Done", onPress: () => navigate("Home") },
        ],
  ...
  return (
    <SafeAreaView style={styles.screen}>
      ...
      >
        <TouchableOpacity
          onPress={() => setAutoReply((v) => !v)}
          style={[
            styles.alertBanner,
            {
              borderLeftColor: autoReply ? C.green : C.border,
              backgroundColor: autoReply ? C.greenLight : C.greyLight,
            },
          ]}
        >
          <Text
            style={{
              flex: 1,
              fontSize: 13,
              color: autoReply ? C.green : C.greyMid,
              fontWeight: "600",
            }}
          >
            {autoReply
        ...
        {autoReply && (
          ...
          >
            ...
            <Text
              style={{ fontSize: 13, color: C.textPrimary, lineHeight: 18 }}
            >
        ...
        {[
          ...
          },
          {
            ...
            placeholder: "123 Main St, Nocatee",
            keyboard: "default",
          },
        ].map((f) => (
        ...
        >
          {SOURCES.map((s) => (
            <TouchableOpacity
              ...
              style={[
                styles.filterChip,
                source === s && styles.filterChipActive,
                { marginBottom: 0 },
              ]}
            >
              <Text
        ...
        <TextInput
          ...
          placeholder="Paver type, size estimate, HOA restrictions..."
          placeholderTextColor={C.greyMid}
          multiline
          value={form.notes}
        ...
        />
        <TouchableOpacity
          style={[styles.greenBtn, saved && { opacity: 0.6 }]}
          onPress={handleSave}
          disabled={saved}
        >
          {saved ? (
            ...
            <Text style={styles.greenBtnText}>
              💾 Save Lead{autoReply ? " & Send Auto-Reply" : ""}
            </Text>
          )}
        </TouchableOpacity>
        <TouchableOpacity
  ...
  const CAMPAIGNS = [
    {
      ...
      tag: "Seasonal",
      desc: "Pre-summer sealing before the rain season. 10% off jobs booked by May 31.",
      target: "All active clients",
      count: 24,
      color: C.orange,
      msg: "Hey [Name]! Summer rain season is coming fast. Book your paver sealing now and save 10%. Reply 'BOOK' and we'll get you scheduled. 🌞",
    },
    {
      id: "c2",
      icon: "🏘",
      title: "Neighbor Referral Blast",
      tag: "Referral",
      desc: "Send to clients who got work done 60+ days ago.",
      target: "60-day+ clients",
      count: 8,
      color: C.blue,
      msg: "Hi [Name]! Love how your pavers turned out? Send them our way — you'll both get $50 off. 🙏",
    },
    {
      id: "c3",
      icon: "🎄",
      title: "Holiday Curb Appeal",
      tag: "Holiday",
      desc: "December campaign — clean pavers before holiday gatherings.",
      target: "All clients",
      count: 24,
      color: C.red,
      msg: "Hi [Name]! Hosting this holiday season? Let us make your pavers shine. Book December and save $75. 🎄",
    },
    {
      id: "c4",
      icon: "📅",
      ...
      tag: "Anniversary",
      desc: "Auto-sends on 1-year job anniversary.",
      target: "Year-1 clients",
      count: 6,
      color: C.gold,
      msg: "Hi [Name]! It's been one year since we sealed your pavers. Time for a maintenance check! 📅",
    },
    {
      id: "c5",
      icon: "❄️",
      title: "Off-Season Rate",
      tag: "Promo",
      ...
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false}>
        ...
        {CAMPAIGNS.map((camp) => {
          ...
          return (
            <View
              key={camp.id}
              style={[styles.followUpCard, isSent && { opacity: 0.5 }]}
            >
              <View
                style={{
                  flexDirection: "row",
              ...
              >
                <View
                  style={[
                    styles.followUpIconLg,
                    { backgroundColor: camp.color + "22" },
                  ]}
                >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    → {camp.target} · {camp.count} recipients
                  </Text>
                </View>
              </View>
              <Card
              ...
              >
                <Text
                  style={{ fontSize: 12, color: C.textPrimary, lineHeight: 17 }}
                >
                  {camp.msg}
                </Text>
              </Card>
              {!isSent ? (
                <View style={{ flexDirection: "row", gap: 8 }}>
                  <TouchableOpacity
                    style={[styles.greenBtn, { flex: 1, marginBottom: 0 }]}
                    onPress={() =>
                      Alert.alert(
                        `Send "${camp.title}"?`,
                        `This will text ${camp.count} clients.`,
                        [
                          { text: "Cancel", style: "cancel" },
                          {
                            text: "Send",
                            onPress: () => setSent((s) => [...s, camp.id]),
                          },
                        ],
                      )
                    }
...
const ReferralTrackingScreen = ({ navigate }) => {
  ...
  const CHAINS = [
    {
      id: "r1",
      referrer: "Mike Thornton",
      referred: "James Duval",
      service: "Paver Sealing · $1,550",
      status: "rewarded",
      ri: "MT",
      ji: "JD",
    },
    {
      id: "r2",
      referrer: "James Duval",
      referred: "Carol Lange",
      service: "Pool Deck · Quote sent",
      status: "pending",
      ri: "JD",
      ji: "CL",
    },
    {
      id: "r3",
      referrer: "Paul Hendricks",
      referred: "Dan Marsh",
      service: "Walkway Seal · $400",
      status: "rewarded",
      ri: "PH",
      ji: "DM",
    },
  ...
  const sCol = (s) =>
    ({ rewarded: C.green, pending: C.orange })[s] || THEMES.rugged.greyMid;
      ...
      <View style={[styles.header, { paddingBottom: 14 }]}>
        <View style={styles.headerRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.headerSub}>April 2026 · all time</Text>
          </View>
        </View>
        <View style={{ flexDirection: "row", gap: 6, marginTop: 12 }}>
          {[
            { val: 11, label: "total refs" },
            { val: 8, label: "jobs closed" },
            { val: "$400", label: "rewards sent" },
            { val: "$12.4k", label: "rev from refs" },
          ].map((k) => (
            <View
              key={k.label}
              style={{
            ...
            >
              <Text
                style={{ color: C.headerText, fontSize: 16, fontWeight: "900" }}
              >
                {k.val}
              </Text>
              <Text
      ...
      >
        <View style={{ padding: 14 }}>
          ...
          {CHAINS.map((c) => (
            <View key={c.id} style={[styles.card, { marginBottom: 10 }]}>
              ...
              >
                ...
                <View
                  style={{
                    backgroundColor: sCol(c.status) + "22",
                    paddingHorizontal: 8,
                    paddingVertical: 4,
                    borderRadius: 8,
                  }}
          ...
          >
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                >
                  "Know any neighbors who could use the same?"
                </Text>
              </View>
          ...
          <TouchableOpacity
            ...
            onPress={() => Alert.alert("Invite sent!")}
          >
...
const MissedCallAutoTextScreen = ({ navigate }) => {
  ...
  const [message, setMessage] = useState(
    "Hey! Sorry I missed your call — I'm out on a job. I'll call you back within the hour. Want a free estimate? Reply YES and I'll text you a link. — HydroSeal Pavers 🌿",
  );
  ...
  const RULES = [
    {
      key: "afterRings",
      label: "Auto-text after 3 rings",
      desc: "Fires if call goes unanswered",
      icon: "📞",
    },
    {
      key: "afterVoicemail",
      label: "Auto-text after voicemail",
      desc: "Fires when voicemail ends",
      icon: "✉️",
    },
    {
      key: "workHours",
      label: "Work hours only",
      desc: "7 AM – 7 PM · Mon–Sat",
      icon: "🕐",
    },
    {
      key: "oncePerNumber",
      label: "Once per number",
      desc: "Don't double-text repeat callers",
      icon: "🔁",
    },
  ];
  return (
    <SafeAreaView style={styles.screenGreen}>
      <View style={styles.header}>
        ...
        <Text style={{ color: `${C.headerText}A6`, fontSize: 13 }}>
          Auto-text fires when you miss a call
        </Text>
        <View
          style={{
            flexDirection: "row",
          ...
          <Card style={{ padding: 0, overflow: "hidden" }}>
            {RULES.map((r, i) => (
              <View key={r.key}>
                {i > 0 && <View style={styles.qbDivider} />}
                <TouchableOpacity
                  style={[styles.qbRow, { justifyContent: "space-between" }]}
                  onPress={() => tog(r.key)}
                >
                  <View
                    style={[
...
const ScheduleScreen = ({ navigate }) => {
  ...
  const [events, setEvents] = useState({
    [fmtDateKey(today)]: [
      {
        id: "e1",
        type: "job",
        title: "Mike Reynolds — Paver Sealing",
        time: "9:00 AM",
        color: "#22C55E",
      },
      {
        id: "e2",
        type: "apt",
        title: "Sandra Ortiz — Quote Walkthrough",
        time: "2:00 PM",
        color: "#3B82F6",
      },
    ],
    [fmtDateKey(addDays(today, 1))]: [
      {
        id: "e3",
        type: "job",
        title: "Sarah Chen — Pressure Wash",
        time: "8:30 AM",
        color: "#22C55E",
      },
    ],
    [fmtDateKey(addDays(today, 3))]: [
      ...
      },
      {
        id: "e5",
        type: "followup",
        title: "Brenda Walsh — Follow-up call",
        time: "3:00 PM",
        color: "#F97316",
      },
    ],
    [fmtDateKey(addDays(today, 5))]: [
      {
        id: "e6",
        type: "job",
        title: "Marcus Webb — Driveway Seal",
        time: "10:00 AM",
        color: "#22C55E",
      },
    ],
  ...
  const addEvent = () => {
    ...
    const newEvent = {
      ...
      type: eventType,
      title: eventForm.title || `New ${eventType}`,
                ...
                >
                  ...
                  >
                    <Text
                      style={{
                        fontSize: 13,
                        fontWeight: isSelected || isTodayDay ? "800" : "400",
                        color: isSelected
                          ? C.white
                          : isTodayDay
                            ? C.green
      ...
      <View>
              ...
              >
                <Text
                  style={{
                    fontSize: 10,
                    fontWeight: "700",
                    color: isTodayDay ? C.green : C.greyMid,
                    marginBottom: 4,
                  }}
                >
                  {dayNames[d.getDay()]}
        ...
        <View>
          {[
            ...
            const slotEvents = selectedEvents.filter((e) => {
              const hr = parseInt(e.time);
              const isPM = e.time?.includes("PM") && hr !== 12;
              return (isPM ? hr + 12 : hr) === ti + 8;
            });
            return (
              <TouchableOpacity
                key={ti}
                onPress={() => {
                  setEventForm((f) => ({ ...f, time }));
          ...
          {[
            ...
            <TouchableOpacity
              ...
              style={[
                ...
                viewMode === key && { backgroundColor: C.cardBg },
              ]}
            >
              <Text
                style={{
                  fontSize: 12,
                  fontWeight: "700",
                  color: viewMode === key ? C.green : "rgba(255,255,255,0.75)",
                }}
              >
                {label}
              </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { maxHeight: "85%" }]}>
                ...
                >
                  <Text
                    style={{
                      fontSize: 13,
                      fontWeight: "600",
                      color: eventType === key ? C.green : C.grey,
                    }}
                  >
                    {label}
                  </Text>
            ...
            <TextInput
              ...
              placeholder="Client name or event"
              placeholderTextColor={C.greyMid}
            ...
            <TextInput
              style={[
                styles.input,
                { marginBottom: 16, height: 60, textAlignVertical: "top" },
              ]}
              placeholder="Access info, scope, reminders..."
              placeholderTextColor={C.greyMid}
              multiline
              value={eventForm.notes}
              onChangeText={(v) => setEventForm((f) => ({ ...f, notes: v }))}
            />
            <TouchableOpacity style={styles.greenBtn} onPress={addEvent}>
              <Text style={styles.greenBtnText}>Add to Schedule</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.greenBtn,
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { maxHeight: "70%" }]}>
            ...
            {dayEventsFor(selectedDate).length === 0 ? (
              ...
              dayEventsFor(selectedDate).map((event) => (
                <View
                  key={event.id}
                  style={[styles.eventCard, { borderLeftColor: event.color }]}
                >
            ...
            >
              <Text style={styles.greenBtnText}>+ Add to This Day</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
  ...
  const LEADS = [
    {
      ...
      name: "Sandra Ortiz",
      phone: "(904) 555-0199",
      address: "22 Pinecrest Ln, Nocatee",
      time: "Today 8:14 AM",
      source: "Website form",
      notes: "Interested in full driveway + patio seal",
      status: "hot",
    },
    {
      id: "l2",
      name: "Carlos Mendez",
      phone: "(904) 555-0312",
      address: "88 Riverbend Dr, St Johns",
      time: "Yesterday 3:45 PM",
      source: "Facebook",
      notes: "Saw our ad — wants estimate for backyard pavers",
      status: "warm",
    },
    {
      id: "l3",
      name: "Tanya Brooks",
      phone: "(904) 555-0441",
      address: "14 Osprey Ct, Fleming Island",
      time: "2 days ago",
      source: "Nextdoor",
      notes: "Neighbor referral from Sarah Chen",
      status: "warm",
    },
  ];
  const statusCol = (s) =>
    ({ hot: C.red, warm: C.orange, cold: C.blue })[s] || THEMES.rugged.greyMid;
  const statusLbl = (s) =>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View
            style={[
              styles.card,
              { borderLeftWidth: 4, borderLeftColor: statusCol(item.status) },
            ]}
          >
            <View
              style={{
            ...
            >
              ...
              <View
                style={{
                  backgroundColor: statusCol(item.status) + "22",
                  paddingHorizontal: 8,
                  paddingVertical: 3,
                  borderRadius: 10,
                }}
              >
                ...
                >
                  {statusLbl(item.status)}
                </Text>
              </View>
            </View>
            <Text
              style={{ fontSize: 12, color: C.textSecondary, marginBottom: 2 }}
            >
              {item.address}
            </Text>
            <Text
              style={{ fontSize: 12, color: C.textSecondary, marginBottom: 6 }}
            >
...
const BusinessSetupScreen = ({ navigate }) => {
  const { C, styles, appearanceMode, setAppearanceMode } = useAppTheme();
  ...
  const handleLogoUpload = async () => {
    try {
      ...
      if (perm.status !== "granted") {
        Alert.alert("Photos needed", "Allow photo access in Settings.");
        return;
      }
    ...
    <SafeAreaView style={styles.screenGreen}>
      ...
      >
        ...
        >
          ...
          >
            {biz.logo && !biz.logo.startsWith("mock") ? (
              <Image
                source={{ uri: biz.logo }}
              ...
              <Text style={{ fontSize: 28 }}>{biz.logo ? "✅" : "🏢"}</Text>
            )}
          </View>
          <View style={{ flex: 1 }}>
            <Text
              ...
              <View key={theme.key}>
                ...
                >
                  <View style={{ flex: 1 }}>
                    ...
                    >
                      {[
                        THEMES[theme.key].greenDark,
                        THEMES[theme.key].green,
                        THEMES[theme.key].greyLight,
                      ].map((dot, idx) => (
                        <View
                          key={`${theme.key}-${idx}`}
                          style={{
                  ...
                  <Text
                    style={{
                      fontSize: 18,
                      color: active ? C.green : C.greyMid,
                      fontWeight: "700",
                    }}
                  >
                    {active ? "✓" : "○"}
          ...
          {[
            {
              ...
              label: "Referral Program",
              price: "Free",
              desc: "Client earns rewards for referrals",
            },
            {
              key: "careProgram",
              label: "Care Program",
              price: "$9/mo",
              desc: "Annual maintenance subscription",
            },
            {
              key: "holidayCampaigns",
              label: "Holiday Campaigns",
              price: "$5/mo",
              desc: "Auto seasonal outreach",
            },
            {
              key: "missedCallAutoText",
              label: "Missed Call Auto-Text",
              price: "$14/mo",
              desc: "Auto-text fires when you miss a call",
            },
          ].map((t, i) => (
            <View key={t.key}>
              {i > 0 && <View style={styles.qbDivider} />}
                    ...
                    >
                      <Text
                        style={{
                          fontSize: 11,
                          fontWeight: "700",
                          color: biz[t.key] ? C.green : C.greyMid,
                        }}
                      >
                        {t.price}
                      </Text>
        ...
        <TouchableOpacity
          style={[styles.greenBtn, { marginTop: 16 }]}
          onPress={handleSave}
        >
          <Text style={styles.greenBtnText}>
...
const SocialSetupScreen = ({ navigate }) => {
  ...
  const [social, setSocial] = useState({
    gbpLink: BIZ_CONFIG.gbpLink || "",
    instagramHandle: BIZ_CONFIG.instagramHandle || "",
    facebookPage: BIZ_CONFIG.facebookPage || "",
    nextdoorProfile: "",
    googleReviewDefault: true,
    facebookReviewDefault: false,
    autoReviewDay: "3",
  });
    ...
    <SafeAreaView style={styles.screenGreen}>
      ...
      >
            ...
            <View key={f.key}>
              ...
              <View style={styles.qbRow}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.qbRowLabel}>{f.label}</Text>
                  <TextInput
                    style={[styles.qbRowValue, { fontSize: 13 }]}
                    value={social[f.key]}
                    onChangeText={(v) => upd(f.key, v)}
                    placeholder={f.placeholder}
                    placeholderTextColor={C.greyMid}
        ...
        <Card style={{ padding: 0, overflow: "hidden" }}>
          {[
            {
              key: "googleReviewDefault",
              label: "Default: Send to Google",
              desc: "Pre-checked on review request screen",
            },
            {
              key: "facebookReviewDefault",
              label: "Default: Send to Facebook",
              desc: "Pre-checked on review request screen",
            },
          ].map((t, i) => (
            <View key={t.key}>
              ...
              <TouchableOpacity
                style={[styles.qbRow, { justifyContent: "space-between" }]}
                onPress={() => upd(t.key, !social[t.key])}
              >
                <View style={{ flex: 1 }}>
                  <Text
                    style={{
          ...
          <View style={styles.qbRow}>
            ...
            <View style={[styles.accordionInput, { minWidth: 70 }]}>
              <TextInput
                style={[styles.accordionInputField, { minWidth: 30 }]}
                value={social.autoReviewDay}
        ...
        <TouchableOpacity
          style={[styles.greenBtn, { marginTop: 16 }]}
          onPress={() => {
            Object.assign(BIZ_CONFIG, {
              gbpLink: social.gbpLink,
  ...
  const [campaigns, setCampaigns] = useState([
    {
      ...
      month: "March",
      enabled: true,
      msg: "Spring is here! Get your pavers sealed before the summer rush. Book now and save 10%.",
    },
    {
      id: "hc2",
      name: "Summer Prep",
      month: "May",
      enabled: true,
      msg: "Summer rain is coming. Is your sealant ready? Book before June and save.",
    },
    {
      id: "hc3",
      name: "Holiday Curb Appeal",
      month: "November",
      enabled: true,
      msg: "Hosting this holiday season? Let us make your pavers shine before guests arrive.",
    },
    {
      id: "hc4",
      ...
      month: "January",
      enabled: false,
      msg: "Start the new year fresh. Book a January seal job and save $75.",
    },
    {
      id: "hc5",
      name: "Back to School",
      month: "August",
      enabled: false,
      msg: "Summer winding down? Perfect time for a paver seal before fall.",
    },
  ]);
  const toggle = (id) =>
    setCampaigns((prev) =>
    ...
    <SafeAreaView style={styles.screenGreen}>
      ...
      >
        <Card style={{ backgroundColor: C.actionLight, marginBottom: 14 }}>
          ...
          >
            📅 Enabled campaigns auto-send to all active clients on the 1st of
            that month. Toggle on/off anytime.
          </Text>
        </Card>
        {campaigns.map((c) => (
          <View key={c.id} style={[styles.card, { marginBottom: 10 }]}>
  ...
  const NOTIF_ITEMS = [
    {
      key: "newLead",
      label: "New Lead",
      desc: "Form submission or missed call",
    },
    {
      key: "missedCall",
      label: "Missed Call",
      desc: "When a call goes unanswered",
    },
    {
      key: "jobReminder",
      label: "Job Reminder",
      desc: "1 hour before scheduled job",
    },
    {
      key: "followUpDue",
      label: "Follow-Up Due",
      desc: "When a client touchpoint fires",
    },
    {
      key: "quoteExpiring",
      label: "Quote Expiring",
      desc: "3 days before 30-day expiry",
    },
    {
      key: "reviewReceived",
      label: "Review Received",
      desc: "New Google or Facebook review",
    },
    {
      key: "paymentReceived",
      label: "Payment Received",
      desc: "Invoice paid confirmation",
    },
  ];
  return (
    <SafeAreaView style={styles.screenGreen}>
      ...
      >
        ...
        <TouchableOpacity
          style={[styles.greenBtn, { marginTop: 16 }]}
          onPress={() => Alert.alert("✅ Notification preferences saved!")}
        >
    ...
    <SafeAreaView style={styles.screenGreen}>
      ...
      >
        ...
        >
          ...
          >
            {[
              ...
              "Drag flags to fine-tune corners",
              "Auto-calculates sq ft from GPS coords",
              "Prefills Quote Builder instantly",
            ].map((item, i) => (
              <View
                key={i}
                style={{
          ...
          >
            <Text
              style={{ color: C.headerText, fontSize: 15, fontWeight: "800" }}
            >
              Open Satellite Map →
            </Text>
          </View>
        ...
        <TouchableOpacity
          style={[
            styles.card,
            { borderWidth: 2, borderColor: C.orange + "88", marginBottom: 14 },
          ]}
        ...
        >
          ...
          >
            {[
              "Works on-site without internet",
              "Tap corners in live camera view",
              "Enter one known reference length",
              "Calculates area via pixel ratio",
            ].map((item, i) => (
              <View
                key={i}
                style={{
              ...
              >
                <Text
                  style={{ color: C.orange, fontSize: 12, fontWeight: "700" }}
                >
                  ✓
                </Text>
                <Text style={{ fontSize: 12, color: C.textPrimary }}>
  ...
  const calcArea = () => {
    if (flags.length < 3) {
      Alert.alert(
        ...
        "Tap the map to place corner flags.",
      );
      return;
    }
  ...
  const handleSaveToQuote = () => {
    if (!sqft) {
      Alert.alert(
        ...
        'Tap "Finish Area" to calculate sq ft.',
      );
      return;
    }
  ...
  return (
    <SafeAreaView style={styles.screenGreen}>
      <View style={styles.header}>
        <View style={styles.headerRow}>
          <TouchableOpacity
            onPress={() => navigate("MeasureEntry", { customer })}
            style={styles.backBtn}
          >
            <Text style={styles.backArrow}>‹</Text>
          </TouchableOpacity>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.headerSub}>
              {flags.length} flags placed{sqft ? ` · ${fmtSqFt(sqft)}` : ""}
            </Text>
          </View>
          {flags.length > 0 && (
            <TouchableOpacity
      ...
      >
        {AREA_TYPES.map((t) => (
          <TouchableOpacity
            ...
            style={[
              styles.filterChip,
              areaType === t && styles.filterChipActive,
              { marginRight: 0 },
            ]}
          >
            <Text
      ...
      <View style={{ flex: 1, position: "relative" }}>
        {mapAvailable ? (
          ...
          >
            ...
            >
              React Native Maps renders here on a physical device or Android
              Expo Go.{"\n\n"}
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                setFlags([
                  { latitude: 30.109, longitude: -81.426 },
                  { latitude: 30.109, longitude: -81.425 },
                  { latitude: 30.1085, longitude: -81.425 },
                  { latitude: 30.1085, longitude: -81.426 },
                ]);
                setSqft(850);
              }}
            >
        ...
        {mapAvailable && flags.length === 0 && (
          ...
          >
            ...
            >
              Tap corners of the {areaType.toLowerCase()} to place flags
            </Text>
          </View>
        )}
...
const MeasureCameraScreen = ({ navigate, params }) => {
  ...
  const nudge = (dx, dy) => {
    ...
    setPoints((prev) =>
      prev.map((p, i) =>
        i === selectedPoint ? { x: p.x + dx, y: p.y + dy } : p,
      ),
    );
    setSqft(null);
  };
  ...
  const calcPixelArea = () => {
    if (points.length < 3) {
      Alert.alert("Need 3+ points", "Tap corners on the camera view.");
      return;
    }
    if (!refWidth || parseFloat(refWidth) <= 0) {
  ...
  const handleSaveToQuote = () => {
    if (!sqft) {
      Alert.alert("Calculate area first");
      return;
    }
    navigate("QuoteBuilder", {
      customer,
  ...
  if (permLoading)
    return (
      ...
      >
        ...
        <Text style={{ color: C.headerText, marginTop: 12 }}>
  ...
  return (
    <SafeAreaView style={styles.screenGreen}>
      ...
      >
        {cameraAvail && permission && CameraViewComp ? (
          ...
          >
            ...
            >
              {!cameraAvail
                ? "Install: npx expo install expo-camera react-native-svg"
                : "Camera permission required — tap the tap area below to simulate"}
            </Text>
          </View>
        )}
        ...
        {points.length === 0 && (
          ...
          >
            ...
            >
              👆 Tap corners of the {areaType.toLowerCase()} · First 2 points =
              reference edge
            </Text>
          </View>
        )}
        ...
        {points.length === 2 && (
          ...
          >
            ...
            >
              📏 Yellow edge = reference. You'll enter this length in feet to
              calculate area.
            </Text>
          </View>
        )}
      ...
      {/* Bottom controls */}
      <ScrollView
        style={{ maxHeight: 220, backgroundColor: C.cardBg }}
        ...
        <View style={{ flexDirection: "row", gap: 10 }}>
          {points.length > 0 && (
            <TouchableOpacity
              style={[
                styles.greenBtn,
                { flex: 0, marginBottom: 0, paddingHorizontal: 14 },
              ]}
              onPress={() => {
      ...
      <Modal visible={showRefModal} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            ...
            >
              The yellow dashed line between your first two points is the
              reference edge.{"\n"}
...
export default function App() {
  ...
  const C = useMemo(() => THEMES[appearanceMode], [appearanceMode]);
  ...
  const barStyle = greenHeaderScreens.includes(screen)
    ? "light-content"
    : "dark-content";
  const barBg = C.primaryBg;

  const screenMap = {
    Home: <HomeScreen navigate={navigate} params={params} />,
    Customers: <CustomersScreen navigate={navigate} customers={customers} />,
    CustomerDetail: (
      <CustomerDetailScreen navigate={navigate} params={params} />
    ),
    NewCustomer: (
      <NewCustomerScreen navigate={navigate} addCustomer={addCustomer} />
    ),
    QuoteBuilder: (
      <QuoteBuilderScreen
        navigate={navigate}
    ...
    MeasureSatellite: (
      <MeasureSatelliteScreen navigate={navigate} params={params} />
    ),
  ...
  return (
    <ThemeContext.Provider
      value={{ C, styles, appearanceMode, setAppearanceMode }}
    >
      <StatusBar
        barStyle={barStyle}
        translucent={false}
...
const createStyles = (C) =>
  StyleSheet.create({
    ...
    clientName: { fontSize: 14, fontWeight: "700", color: C.textPrimary },
    ...
    jobStatVal: { fontSize: 14, fontWeight: "800", color: C.headerText },
    templateCard: {
      flex: 1,
      height: 90,
      borderRadius: 12,
    ...
    detailAvatarText: { fontSize: 26, fontWeight: "800", color: C.headerText },
    detailName: {
      fontSize: 22,
      fontWeight: "900",
      color: C.headerText,
    ...
    detailAddr: { fontSize: 12, color: `${C.headerText}A6`, marginTop: 3 },
    detailActions: {
      backgroundColor: C.cardBg,
      flexDirection: "row",
      borderBottomWidth: 1,
